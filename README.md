...
The API runs on Railway, project `readyyet`, with two environments: `staging` deploys the `staging` branch, `production` deploys `main`, each commit only once CI has passed on it. Changes go to `staging` first, by a squash-merged PR. Releasing is a PR from `staging` into `main`, merged once staging runs that exact commit and `/health` answers 200 (the `promotion` job). Both branches only take PRs with CI green, see ADR 0023.
